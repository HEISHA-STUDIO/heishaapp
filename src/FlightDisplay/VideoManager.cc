...
VideoManager::setToolbox(QGCToolbox *toolbox)
{
   ...
   QString videoSource = VideoSettings::videoSourceRTSP;// _videoSettings->videoSource()->rawValue().toString();
   connect(_videoSettings->videoSource(),   &Fact::rawValueChanged, this, &VideoManager::_videoSourceChanged);
   connect(_videoSettings->udpPort(),       &Fact::rawValueChanged, this, &VideoManager::_udpPortChanged);
   connect(_videoSettings->rtspUrl(),       &Fact::rawValueChanged, this, &VideoManager::_rtspUrlChanged);
   connect(_videoSettings->tcpUrl(),        &Fact::rawValueChanged, this, &VideoManager::_tcpUrlChanged);
   connect(_appSettings,               &AppSettings::send_camera_ip, this, &VideoManager::_rtspUrlChanged);
...
VideoManager::_updateSettings()
{
    ...
    if (_videoSettings->videoSource()->rawValue().toString() == VideoSettings::videoSourceUDP)
        ...
        */
     QUrl _url("rtsp://192.168.124.201:1554/image-t");

     quint16   port_temp =_appSettings->allport();
     QString address_temp = _appSettings->allhost();
     QString topic_temp = _appSettings->alltopic();
     _url.setHost(address_temp);
     _url.setPort(port_temp);
     _url.setUserName(topic_temp);

      qWarning() <<"VideoManager::_updateSettings()" << _url << endl;
    _videoReceiver->setUri(_url.toString());
}
