...
QString VideoReceiver::get_mqtt_host(){
        return _mqtt_host;
}
qint32 VideoReceiver::get_mqtt_port(){
    return _mqtt_port;
}
//-----------------------------------------------------------------------------
#if defined(QGC_GST_STREAMING)
...
VideoReceiver::_timeout()
{
    ...
    if(_videoSettings->streamEnabled()->rawValue().toBool()) {
     ...
     //   _uri = "rtsp://127.0.0.1:1554/live";
         QUrl url(_uri);
   //     _mqtt_port= url.port();
    //    _mqtt_host=url.host();

     //-- If RTSP and no port is defined, set default RTSP port (554)
    //    _rtsplink = new RTSPLink();
    //    _rtsplink->_host=url.host();
     //   _rtsplink->_port=url.port();
    //    emit  _rtsplink->rtsp_run();


       // if(_uri.contains("rtsp://") && url.port() <= 0) {
        qCritical() << "VideoReceiver::start()1" << url.host() << url.port();
        if(_uri.contains("rtsp://")) {
             url.setPort(1554); //554
...
//davis end
    if(!_videoSettings->streamEnabled()->rawValue().toBool() ||
       !_videoSettings->streamConfigured()) {
        ...
        qCritical() <<  _videoSettings->streamEnabled()->rawValue().toBool() <<  _videoSettings->streamConfigured() ;
        return;
    }
  //    qCritical() << "_rtsplink start()111";
#if defined(QGC_GST_STREAMING)
    _stop = false;
 ...
 //      qCritical() << "_rtsplink start()112"  << _uri;
    bool running = false;
    ...
    do {
        ...
        if(isUdp) {
            ...
            //davis start
                 _uri_buf = "rtsp://127.0.0.1:1554/live.sdp";

            g_object_set(G_OBJECT(dataSource), "location", qPrintable(_uri_buf), "latency", 17, "udp-reconnect", 1, "timeout", static_cast<guint64>(5000000), NULL);
            //davis end
    ...
    //davis
         if(!(_uri==_uri_mqtt)){
           ...
           if(rtsp_num==0){
               _rtsplink = new RTSPLink();
               _rtsplink->start();
            //   _rtsplink->start(QThread::HighestPriority);
               rtsp_num=10;
           }
           _rtsplink->_host= url.host();
           _rtsplink->_port=url.port();
           _rtsplink->_image_top=url.userName();
