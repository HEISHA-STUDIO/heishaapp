...
#ifdef _WIN32
#	ifdef myserverpp_EXPORTS
#		define mspp_EXPORT  __declspec(dllexport)
#	else
#		define mspp_EXPORT  __declspec(dllimport)
#	endif
#else
#	define mspp_EXPORT
#endif
...
mspp_EXPORT const char *strerror(int ms_errno);
mspp_EXPORT const char *connack_string(int connack_code);
mspp_EXPORT int sub_topic_tokenise(const char *subtopic, char ***topics, int *count);
mspp_EXPORT int sub_topic_tokens_free(char ***topics, int count);
mspp_EXPORT int lib_version(int *major, int *minor, int *revision);
mspp_EXPORT int lib_init();
mspp_EXPORT int lib_cleanup();
mspp_EXPORT int topic_matches_sub(const char *sub, const char *topic, bool *result);
mspp_EXPORT int validate_utf8(const char *str, int len);
mspp_EXPORT int subscribe_simple(
		...
		const struct libmyserver_will *will=NULL,
		const struct libmyserver_tls *tls=NULL);

mspp_EXPORT int subscribe_callback(
 ...
 * Class: myserverpp
 *
 * A myserver client class. This is a C++ wrapper class for the myserver C
 * library. Please see myserver.h for details of the functions.
 */
class mspp_EXPORT myserverpp {
	private:
		struct myserver *m_ms;
	public:
		myserverpp(const char *id=NULL, bool clean_session=true);
		virtual ~myserverpp();
		...
		int opts_set(enum ms_opt_t option, void *value);
		...
		virtual void on_message(const struct myserver_message * /*message*/) {return;}
