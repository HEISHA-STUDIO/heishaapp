...
//sockfd_start:
...
#endif
   // printf("rtsp_demo_handle\n");

	d = __alloc_demo();
	if (NULL == d) {
      //   printf("rtsp_demo_handle11\n");
         return 1;
		return NULL;
    }
    ...
    inaddr.sin_addr.s_addr =  inet_addr("127.0.0.1"); //htonl(INADDR_ANY);
    inaddr.sin_port = htons(port);

 //   bind(sockfd, (struct sockaddr*)&inaddr, sizeof(inaddr));
 ...
 //    return sk_errno();
    ...
    //    printf("rtsp_demo_handle44\n");
    //   return 4;
		return NULL;
	}

	d->sockfd = sockfd;
    printf("rtsp_demo_handle55\n");
...
#ifdef __LINUX__
        int sndbufsiz = 1024 * 512;  // 1024 * 512;
		int keepalive = 1;
		int keepidle = 60;
		int keepinterval = 3;
		int keepcount = 5;
...
int rtsp_set_video (rtsp_session_handle session, int codec_id, const uint8_t *codec_data, int data_len)
{
	...
	s->vrtpe.seq = 0;
	s->vrtpe.ssrc = 0;
    s->vrtpe.sample_rate =90000; //90000;
...
/*
rtsp  \B4\A6\C0\ED rtsp\C1\AC\C7\EB\C7\F3
*/
...
int rtsp_do_event (rtsp_demo_handle demo)
{
    ...
    //rtp rtcp \B5ķ\A2\CB\CD \D3\EB\BD\D3\CAռ\E0\CC\FD
	...
	while (cc) {//\B4\E6\D4\DArtsp_client_connection
		struct rtsp_client_connection *cc1 = cc;
		struct rtsp_session *s = cc1->session;  //\B4\CB\C1\AC\BDӵ\C4rtsp session
		struct rtp_connection *vrtp = cc1->vrtp;//\B4\CB\C1\AC\BDӵ\C4rtp session \CA\D3Ƶ
		struct rtp_connection *artp = cc1->artp;
		cc = TAILQ_NEXT(cc, demo_entry);//\D5\D2һ\B8\F6rtsp_client_connection
