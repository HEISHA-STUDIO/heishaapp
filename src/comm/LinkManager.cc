...
void LinkManager::setToolbox(QGCToolbox *toolbox)
{
    ...
    connect(_appSettings, &AppSettings::send_com_ip, this, &LinkManager::usr_connectLink);
...
{
      qWarning() <<  "createConnectedLink config " ;
...
{
    qWarning() <<  "createConnectedLink SharedLinkConfigurationPointer " ;
    if (!config) {
...
#endif
        //davis
            case LinkConfiguration::TypeMyTcp:
               pLink = new MyTCPLink(config);
                break;
        //end
...
{
     qWarning() <<  "createConnectedLink LinkManager " ;
    if (name.isEmpty()) {
...
void LinkManager::usr_connectLink(void){
   //   qWarning() << "usr_connectLink";
      quint16   port_temp =_appSettings->allport();
      QString address_temp = _appSettings->allhost();
      QString topic_temp = _appSettings->alltopic();

    bool foundMyTCP = false;
    for (int i=0; i<_sharedLinks.count(); i++) {
        LinkConfiguration* linkConfig = _sharedLinks[i]->getLinkConfiguration();
        if (linkConfig->type() == LinkConfiguration::TypeMyTcp && linkConfig->name() == "MyTCP Link 139.199.24.138:9002:d92c5b2831f237fa") {
            foundMyTCP = true;
            break;
        }
    }
    if(foundMyTCP){
            disconnectLink(_link);
    }
   ...
   if(!foundMyTCP){
        MyTCPConfiguration* MytcpConfig = new MyTCPConfiguration("MyTCP Link 139.199.24.138:9002:d92c5b2831f237fa");
      // udpConfig->setDynamic(true);
        MytcpConfig->setDynamic(true);
        SharedLinkConfigurationPointer config = addConfiguration(MytcpConfig);
        _link = createConnectedLink(config);
        _link->write_topic(topic_temp);
        _link->write_host(address_temp);
        _link->write_port(port_temp);
...
void LinkManager::update_connectLink(void){
   //   qWarning() << "usr_connectLink";
      quint16   port_temp =_appSettings->allport();
      QString address_temp = _appSettings->allhost();
      QString topic_temp = _appSettings->alltopic();
...
{
      qWarning() << "Link Configuration" << "has an empty name." ;
  ...
  //davis
   ...
   //davis end
    ...
    if(settings.contains(LinkConfiguration::settingsRoot() + "/count")) {
          qWarning() << "Link Configuration" << "settings" ;
                ...
                if((LinkConfiguration::LinkType)type < LinkConfiguration::TypeLast) {
                    if(settings.contains(root + "/name")) {
                        ...
                        if(!name.isEmpty()) {
                            ...
                            type = LinkConfiguration::TypeMyTcp;  //davis
                            ...
                            case LinkConfiguration::TypeMock:
                                ...
                                //davis
                            case LinkConfiguration::TypeMyTcp:
                                pLink = dynamic_cast<LinkConfiguration*>(new MyTCPConfiguration(name));
                                break;
                                //end
                            default:
                            case LinkConfiguration::TypeLast:
...
{
  //   qWarning() << "_updateAutoConnectLinks " << _connectionsSuspended << qgcApp()->runningUnitTests() << endl;
...
//     qWarning() << "_autoConnectSettings " << !foundUDP  << _autoConnectSettings->autoConnectUDP()->rawValue().toBool() << endl;
...
/*
 ...
 //    qWarning() << "_sharedAutoconnectConfigurations " << !_sharedAutoconnectConfigurations.count() << endl;
    ...
    foreach (QGCSerialPortInfo portInfo, portList) {
     ...
     //      qWarning() << "QGCSerialPortInfo " << portList << endl;
        ...
        if (portInfo.getBoardInfo(boardType, boardName)) {
            ...
            if (_autoconnectConfigurationsContainsPort(portInfo.systemLocation()) || _autoConnectRTKPort == portInfo.systemLocation()) {
                ...
                if (pSerialConfig) {
               //      qWarning() << "_updateAutoConnectLinks  pSerialConfig  "  << endl;
...
//     qWarning() << "_updateAutoConnectLinks  end  " << endl;
#ifndef __android__
    // Android builds only support a single serial connection. Repeatedly calling availablePorts after that one serial
    // port is connected leaks file handles due to a bug somewhere in android serial code. In order to work around that
    ...
    if (config) {
        ...
        if (config->name() == "Unnamed") {
            ...
            case LinkConfiguration::TypeMock:
                ...
                //davis
            case LinkConfiguration::TypeMyTcp: {
                MyTCPConfiguration* tconfig = dynamic_cast<MyTCPConfiguration*>(config);
                if(tconfig) {
                    config->setName(
                        QString("MYTCP Link %1:%2:%3").arg(tconfig->address().toString()).arg(static_cast<int>(tconfig->myport())).arg(tconfig->mytopic()));
                }
            }
                break;
                //end
...
void LinkManager::_activeLinkCheck(void)
{
    ...
    bool found = false;
     qWarning() << "_activeLinkCheck  " << _activeLinkCheckList.count() << endl;
    ...
    if (_activeLinkCheckList.count() == 0) {
     ...
     qWarning() << "_activeLinkCheck  " << !found  << link << endl;
    if (!found && link) {
