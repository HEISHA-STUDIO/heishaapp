...
Q_DECLARE_LOGGING_CATEGORY(ChargePadLog)

class ChargePad:  public QGCTool
{
    Q_OBJECT

public:
    ChargePad(QGCApplication* app, QGCToolbox* toolbox);

    void setToolbox(QGCToolbox* toolbox) override;
    Q_INVOKABLE void blars_Lock(void);
    Q_INVOKABLE void blars_UNlock(void);
    Q_INVOKABLE void charge(void);
    Q_INVOKABLE void drone(void);
    Q_INVOKABLE void rc(void);
    Q_INVOKABLE void uncharge(void);
    Q_INVOKABLE void undrone(void);
    Q_INVOKABLE void unrc(void);
public slots:
  //  void followMeHandleManager  (const QString&);

private slots:
 //   void _setGPSLocation        (QGeoPositionInfo geoPositionInfo);
    void _sendGCSChargePadReport   ();
...
private:
  //  QElapsedTimer runTime;
    ...
    struct chargePadReport_s {
     //   uint32_t timestamp;     // time since boot
        uint8_t contorl_uint;
